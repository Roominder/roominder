...
class SecureUsersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create secure_user" do
    assert_difference('SecureUser.count') do
      post secure_users_url, params: { secure_user: { email: @secure_user.email, name: @secure_user.name, password: 'secret', password_confirmation: 'secret', username: @secure_user.username } }
    end
  ...
  test "should update secure_user" do
    patch secure_user_url(@secure_user), params: { secure_user: { email: @secure_user.email, name: @secure_user.name, password: 'secret', password_confirmation: 'secret', username: @secure_user.username } }
